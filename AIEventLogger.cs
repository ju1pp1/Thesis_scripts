...
{
    public static string SystemTag = "FSM";
    /*
    ...
    {
        Debug.Log($"[AI][{SystemTag}] t={Time.time:0.00} npc={bb.name} from={from} to={to} " + $"hp={bb.HealthPct:0.00} dist={bb.DistanceToPlayer:0.0} los={(bb.HasLoS ? 1 : 0)} inLookRadius={(bb.InLookRadius ? 1 : 0)}");
    ...
    {
        Debug.Log($"[AI][{SystemTag}] t={Time.time:0.00} npc={bb.name} action={action} " + $"hp={bb.HealthPct:0.00} dist={bb.DistanceToPlayer:0.0} los={(bb.HasLoS ? 1 : 0)} inLookRadius={(bb.InLookRadius ? 1 : 0)}");
